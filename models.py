...
from keras.layers import Bidirectional, concatenate, Dense, Dropout, Input, LSTM, Reshape, TimeDistributed
...
class LSTM_Keras:
    def __init__(self, V, D, seq_len, labels_len, n_hidden_rnn = 32, 
    ...
    def _build(self):        
        ...
        lstm1 = LSTM(self.n_hidden_rnn, return_sequences = True)(embedding1)
        timedist1 = Reshape([self.seq_len])(TimeDistributed(Dense(1))(lstm1))
        dense1 = Dense(self.labels_len)(timedist1)
        model1 = Model(inputs = input_correct_seq, outputs = dense1)                

        embedding2 = Embedding(self.V, self.D, input_length = self.seq_len)(input_incorrect_seq)
        lstm2 = LSTM(self.n_hidden_rnn, return_sequences = True)(embedding2)
        timedist2 = Reshape([self.seq_len])(TimeDistributed(Dense(1))(lstm2))
        dense2 = Dense(self.labels_len)(timedist2)
        model2 = Model(inputs = input_incorrect_seq, outputs = dense2)

        lstm3 = LSTM(self.n_hidden_rnn, return_sequences = True)(input_correct_pos)
        timedist3 = Reshape([self.seq_len])(TimeDistributed(Dense(1))(lstm3))
        dense3 = Dense(self.labels_len)(timedist3)
        model3 = Model(inputs = input_correct_pos, outputs = dense3)

        lstm4 = LSTM(self.n_hidden_rnn, return_sequences = True)(input_incorrect_pos)
        timedist4 = Reshape([self.seq_len])(TimeDistributed(Dense(1))(lstm4))
        dense4 = Dense(self.labels_len)(timedist4)
        model4 = Model(inputs = input_incorrect_pos, outputs = dense4)

        combine1 = Reshape([self.labels_len, 1])(model1.output)
        combine2 = Reshape([self.labels_len, 1])(model2.output)
        combine3 = Reshape([self.labels_len, 1])(model3.output)
        combine4 = Reshape([self.labels_len, 1])(model4.output)
        combined = concatenate([combine1, combine2, combine3, combine4])
        
        merged_lstm = LSTM(self.n_hidden_rnn, return_sequences = True)(combined)
                
        preds_incorrect = TimeDistributed(Dense(1))(merged_lstm)
        preds_correct = TimeDistributed(Dense(1))(merged_lstm)

        output_list = [preds_incorrect, preds_correct]
        model = Model(inputs = input_list, outputs = preds_incorrect)
        optimizer = Adam(lr = self.lr)
        model.compile(optimizer = optimizer, loss = self.loss_fun, metrics=['acc'])
        self.model = model
